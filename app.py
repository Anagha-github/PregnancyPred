...
def run():
    ...
    HR = st.number_input('Heart rate', min_value = 50.0, max_value = 200.0, value = 111.0, step=1e-2)
    if HR <=63 or HR >= 113:
        st.error('Your heart rate is abnormal')
    RR = st.number_input('Respiratory rate', min_value = 5.0, max_value= 30.0, value = 20.0, step=1e-2)
    if RR <=7 or RR >= 24:
    ...
    BT = st.number_input('Body temperature (`C)', value =36.0, step=1e-2)
    if BT <= 35 or BT >= 38:
        st.error('Your body temparature is abnormal')
    GDM = st.number_input('Fasting Glucose', value = 75.0, step=1e-2)
